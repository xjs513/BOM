...
def update(c_id, bom_name, bom_code, com_name, com_code, com_cnt, com_unit, com_unit_price, com_rate):
    conn = get_conn()
    cur = conn.cursor()
    res = dict()
    # def check_update_exist(cur, c_id, bom_code, com_code):
    if check_update_exist(cur, c_id, bom_code, com_code):
        ...
        sql = "update " + table_name + " set bom_name='%s', bom_code='%s',com_name='%s', com_code='%s'," \
               "com_cnt=%f, com_unit='%s', com_unit_price=%f, com_rate=%d where id=%d" \
              % (bom_name, bom_code, com_name, com_code, float(com_cnt),
                 com_unit, float(com_unit_price), int(com_rate), int(c_id))
        print(sql)
        cur.execute(sql)
        conn.commit()
        total = cur.rowcount
